...
一个使用 React + Vite + TypeScript 构建的个人作品集与工作归档网站，支持项目资料管理、文件化笔记、内容在线编辑和文档上传。

## 当前功能

- **首页** — 清爽个人介绍首屏、精选项目与置顶/近期灵感入口
- **关于我** — 个人画像、教育/工作经历、技能面板，支持在线编辑并持久化到 localStorage
- **项目** — 项目卡片网格展示、独立详情页、项目专属工作空间，支持项目文件上传、预览、下载、关联与移至共享库
- **文档库** — 全站文档/技能文件管理，支持拖拽上传、粘贴文字保存到 `notes/documents/`、文本编辑、类型筛选、下载、复制、删除
...
└── models/                   # 静态图片 / 模型资源
server/
└── index.js                  # Express API 服务器（文件上传/下载、笔记 CRUD）
notes/                        # 灵感 Markdown 文件存储（含 _metadata.json 置顶信息，documents/ 存放可 Git 同步的文本资料）
